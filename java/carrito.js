...
function actualizarCarrito() {
    ...
    carrito.items.forEach(item => {
        ...
        itemElement.innerHTML = `
            <img src="${item.imagen}" alt="S/.${item.nombre}" class="item-imagen">
            <div class="item-info">
    ...
    document.querySelector('.subtotal-precio').textContent = `S/.${carrito.subtotal.toFixed(2)}`;
    document.querySelector('.descuento-monto').textContent = `-S/.${carrito.descuento.toFixed(2)}`;
    document.querySelector('.total-precio').textContent = `S/.${carrito.total.toFixed(2)}`;
...
function actualizarContadorCarrito() {
  ...
  document.getElementById('formPago').addEventListener('submit', function (e) {
    ...
    setTimeout(() => {
      window.location.href = "inicio.html"; // Cambia si tu archivo se llama distinto
    }, 3000);
  });
